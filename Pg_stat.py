# pg_stat
# Скрипт для обновления статистики баз postgresql с помощью команды ANALYZE.
# Version: 1.00
...
def update_statistics(pg_stat_cmd, host, port, username, db_name, verbose):
    command = [
        pg_stat_cmd,
        '--host', host,
        '--port', str(port),
    ...
    try:
        ...
        msg = f"Updating statistics for database '{db_name}' completed successfully."
        log_message(msg)  # Записываем в лог краткую информацию
        if verbose:
            print(msg)
        return True
    except subprocess.CalledProcessError as e:
        msg = f"Error updating statistics for database '{db_name}': {e.output.decode() if hasattr(e, 'output') else str(e)}"
...
def main():
    ...
    # Выполнение обновления статистики для каждой базы данных
    for db in databases:
        success = update_statistics(settings['pg_stat_cmd'], settings['host'],
        ...
        if success:
            if settings.get('telegram_notifications_enabled'):
                send_telegram_notification(settings['telegram_script_path'], "r",
                                            f"{settings.get('prefix', '')} {host_name}. Updating statistics for '{db}' completed successfully.")
        else:
            if settings.get('telegram_notifications_enabled'):
                send_telegram_notification(settings['telegram_script_path'], "a",
                                            f"{settings.get('prefix', '')} {host_name}. Updating statistics for '{db}' failed.")
